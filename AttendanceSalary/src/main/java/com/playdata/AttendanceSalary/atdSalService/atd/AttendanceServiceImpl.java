...
public class AttendanceServiceImpl implements AttendanceService {
  ...
  public BigDecimal calculateMonthlyOvertimeHours(String employeeId, YearMonth yearMonth) {
    ...
    BigDecimal totalOvertime = attendanceDAO.getTotalOvertimeHoursByEmployeeAndDateRange(employeeId,
        startDate, endDate);
    log.info("직원 ID: {}의 {}월 연장 근무 시간 합계: {}", employeeId, yearMonth.getMonthValue(),
        totalOvertime);
    return totalOvertime;
  }

  @Override
  public AttendanceEntity checkIn(String employeeId, String companyCode)
...
*/
    // 근무 시간 및 연장 근무 시간 저장
    attendance.setWorkHours(BigDecimal.valueOf(duration.toMinutes()).max(BigDecimal.ZERO)
        .divide(BigDecimal.valueOf(60), 3, RoundingMode.HALF_UP)); // 총 근무 시간 (분 단위)
